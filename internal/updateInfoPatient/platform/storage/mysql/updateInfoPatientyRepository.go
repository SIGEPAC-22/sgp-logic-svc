...
func (u *UpdateInfoPatientRepository) SelectInfoPatientRepo(ctx context.Context, id int) (updateInfoPatient.SelectInfoPatientResponse, error) {
	rows := u.db.QueryRowContext(ctx, "SELECT pat_id_document_type, pat_id_department, pat_id_country, pfl_pregnant FROM sgp_info_svc.pat_patient\ninner join pfl_patient_file AS pfl\non pfl_id_patient = pat_id_patient\nWHERE pat_id_patient = ?;", id)
	...
	if err := rows.Scan(&respDB.DocumentType, &respDB.Department, &respDB.Foreign, &respDB.Pregnant); err != nil {
